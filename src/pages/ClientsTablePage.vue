...
export default {
  ...
  methods: {
    selectRow(row) {
      // Find index of the clicked row in the selected array
      const index = this.selected.findIndex(
        (selectedRow) => selectedRow.client_id === row.client_id
      );

      // If the row is not in the selected array, add it, else remove it
      if (index === -1) {
        this.selected.push(row); // Add row to selected array
      } else {
        this.selected.splice(index, 1); // Remove row from selected array
      }
    },
    rowClassName(row) {
      console.log(row, this.selected); // Debugging line
