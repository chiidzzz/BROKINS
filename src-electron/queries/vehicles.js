...
function addVehicle(vehicle, callback) {
  const query = `
    INSERT INTO Vehicles (plate_number, client_id, reference_name, make_id, model_id, type_id, horsepower, year_built, usage_id, engine_number, chassis_number, notes, data_saved_by)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `;
  db.run(
    query,
...
function editVehicle(vehicle, callback) {
  const query = `
    UPDATE Vehicles
    SET client_id = ?, reference_name = ?, make_id = ?, model_id = ?, type_id = ?, horsepower = ?, year_built = ?, usage_id = ?, engine_number = ?, chassis_number = ?, data_saved_by = ?, notes = ?, plate_number =?
    WHERE vehicle_id = ?
  `;
  db.run(
    query,
